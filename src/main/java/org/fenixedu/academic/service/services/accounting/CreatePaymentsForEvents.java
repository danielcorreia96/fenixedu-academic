...
public class CreatePaymentsForEvents {
    ...
    @Atomic
    public static void run(final User responsibleUser, final Collection<EntryDTO> entryDTOs, final PaymentMode paymentMode, final DateTime whenRegistered) {
        ...
        entryDTOsByEvent.forEach((event, entries) -> {
            event.process(responsibleUser, entries, new AccountingTransactionDetailDTO(whenRegistered, paymentMode));
        });
    }
